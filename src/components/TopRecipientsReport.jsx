...
const TopRecipientsReport = () => {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-sky-100 bg-gradient-to-br from-white to-slate-50 p-6">
      <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-teal-400 to-emerald-600" />

      <div className="mb-4 flex items-center gap-2">
        <span className="inline-flex h-7 w-7 items-center justify-center rounded-md bg-sky-50 text-sky-600">🏢</span>
        <h2 className="text-xl font-bold text-slate-900">Top Grant Recipients</h2>
      </div>
      ...
      <div className="mt-6">
        <div className="mb-2 flex items-center gap-2">
          <span className="inline-flex h-6 w-6 items-center justify-center rounded bg-sky-50 text-sky-600">📈</span>
          <h3 className="text-lg font-semibold text-slate-900">Key Insights</h3>
        </div>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div className="rounded-xl border border-amber-300 bg-amber-50 p-3">
            <div className="text-sm font-semibold text-amber-800">Top Recipient</div>
            <div className="mt-1 text-xl font-bold text-slate-900">{data[0]?.LegalName}</div>
            <div className="text-sm text-slate-700">${data[0]?.totalAmount?.toLocaleString()} across {data[0]?.grantCount} grants</div>
          </div>
          <div className="rounded-xl border border-sky-300 bg-sky-50 p-3">
            <div className="text-sm font-semibold text-sky-800">Total Recipients</div>
            <div className="mt-1 text-xl font-bold text-slate-900">{data.length}</div>
            <div className="text-sm text-slate-700">Organizations receiving grants</div>
          </div>
          <div className="rounded-xl border border-emerald-300 bg-emerald-50 p-3">
            <div className="text-sm font-semibold text-emerald-800">Average Grant Size</div>
            <div className="mt-1 text-xl font-bold text-slate-900">${Math.round((data.reduce((sum, r) => sum + (Number(r.totalAmount) / Number(r.grantCount)), 0) / (data.length || 1)) || 0).toLocaleString()}</div>
            <div className="text-sm text-slate-700">Per grant across all recipients</div>
          </div>
        </div>
      </div>
    </div>
